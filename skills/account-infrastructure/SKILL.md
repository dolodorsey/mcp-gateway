...
description: Inventory, connect, verify, secure, and document the exact accounts, domains, sender identities, social profiles, and provider credentials used by each entity. Direct-first KHG skill. Resolve the exact entity and current backend before execution; preserve brand/entity isolation and verify results.
metadata:
  version: 2.0.0
...
# Account Infrastructure v2

Inventory, connect, verify, secure, and document the exact accounts, domains, sender identities, social profiles, and provider credentials used by each entity.
...
- Prefer direct account/provider connections.
- Store secrets only in approved secret stores; never in tracked files.
- Retire duplicate/stale accounts and record one current source of truth.
...
---
*KHG Account Infrastructure — direct-first, entity-isolated, evidence-verified.*
